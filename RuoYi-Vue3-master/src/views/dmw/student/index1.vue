      ...
      <el-col :span="24" :xs="24">
        ...
        <el-table v-loading="loading" :data="studentList" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="六困生类型" align="center" prop="hardshipType">
            <template #default="scope">
              <dict-tag :options="dmw_hardship_type" :value="scope.row.hardshipType" />
            </template>
          </el-table-column>
          <el-table-column label="最近记录时间" align="center" prop="lastRecordTime" width="180">
            <template #default="scope">
              <span>{{ parseTime(scope.row.lastRecordTime, '{y}-{m}-{d}') }}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="320">
            <template #default="scope">
              ...
              <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['dmw:student:remove']">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
...
import { listStudent, getStudent, delStudent, addStudent, updateStudent } from "@/api/dmw/student";
